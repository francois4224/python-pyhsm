...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generate password AEAD using YubiHSM", add_help=True
    )
    parser.add_argument(
        "-D",
        "--device",
    ...
    parser.add_argument("--validate", dest="validate", help="AEAD to validate.")
    parser.add_argument(
        "--min_length",
        type=int,
        dest="min_len",
...
def validate_aead(hsm, args, password):
    ...
    try:
        ...
        return hsm.validate_aead(
            args.nonce.decode("hex"), args.key_handle, args.validate.decode("hex"), pw
        )
    except pyhsm.exception.YHSM_CommandFailed as e:
        if e.status_str == "YHSM_FUNCTION_DISABLED":
            print(
                "ERROR: The key handle %s is not permitted to do AES ECB compare."
                % (args.key_handle)
            )
            return None
        else:
