...
def input_validate_nonce(nonce, name="nonce", pad=False):
    ...
    if type(nonce) is not bytes:
        raise pyhsm.exception.YHSM_WrongInputType(name, bytes, type(nonce))
    ...
    if pad:
        return nonce.ljust(pyhsm.defines.YSM_AEAD_NONCE_SIZE, chr(0x0).encode())
    else:
        return nonce
...
def input_validate_aead(
    aead, name="aead", expected_len=None, max_aead_len=pyhsm.defines.YSM_AEAD_MAX_SIZE
):
    ...
    if got != expected:
        raise (
            pyhsm.exception.YHSM_Error(
                "Bad %s in response (got %i, expected %i)" % (name, got, expected)
            )
        )
    return got
...
def validate_cmd_response_hex(name, got, expected):
    ...
    if got != expected:
        raise (
            pyhsm.exception.YHSM_Error(
                "Bad %s in response (got 0x%x, expected 0x%x)" % (name, got, expected)
            )
        )
    return got
...
def validate_cmd_response_str(name, got, expected, hex_encode=True):
    ...
    if got != expected:
        raise (
            pyhsm.exception.YHSM_Error(
                "Bad %s in response (got %s, expected %s)" % (name, got, expected)
            )
        )
    return got
