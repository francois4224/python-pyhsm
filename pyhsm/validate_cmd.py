...
class YHSM_Cmd_AEAD_Validate_OTP(YHSM_AEAD_Cmd):
    ...
    def __init__(self, stick, public_id, otp, key_handle, aead):
        ...
        packed = struct.pack(fmt, self.public_id, self.key_handle, self.otp, aead)
        YHSM_AEAD_Cmd.__init__(
            self, stick, pyhsm.defines.YSM_AEAD_YUBIKEY_OTP_DECODE, packed
        )
    ...
    def parse_result(self, data):
        ...
        if self.status == pyhsm.defines.YSM_STATUS_OK:
            self.response = YHSM_ValidationResult(
...
class YHSM_ValidationResult:
    ...
    def __repr__(self):
        return (
            "<%s instance at %s: public_id=%s, use_ctr=%i, session_ctr=%i, ts=%i/%i>"
            % (
