...
def validate_otp(hsm, from_key):
    ...
    return hsm.db_validate_yubikey_otp(
        modhex_decode(public_id).decode("hex"), modhex_decode(otp).decode("hex")
    )
...
def validate_yubikey_with_aead(hsm, from_key, aead, key_handle):
    ...
    return hsm.validate_aead_otp(nonce, otp.decode("hex"), key_handle, aead)
...
def split_id_otp(from_key):
    ...
    if len(from_key) > 32:
        ...
        raise pyhsm.exception.YHSM_Error(
            "Bad from_key length %i < 32 : %s" % (len(from_key), from_key)
        )
