...
class YHSM_Cmd_DB_YubiKey_Store(YHSM_Cmd):
    ...
    def __init__(self, stick, public_id, key_handle, aead, nonce=None):
        ...
        if nonce is None:
            ...
            YHSM_Cmd.__init__(
                self, stick, pyhsm.defines.YSM_DB_YUBIKEY_AEAD_STORE, packed
            )
        else:
            ...
            YHSM_Cmd.__init__(
                self, stick, pyhsm.defines.YSM_DB_YUBIKEY_AEAD_STORE2, packed
            )
    ...
    def parse_result(self, data):
        ...
        public_id, key_handle, self.status = struct.unpack(
            "< %is I B" % (pyhsm.defines.YSM_AEAD_NONCE_SIZE), data
        )
...
class YHSM_Cmd_DB_Validate_OTP(YHSM_Cmd):
    ...
    def parse_result(self, data):
        ...
        public_id, use_ctr, session_ctr, ts_high, ts_low, self.status = struct.unpack(
            fmt, data
        )
