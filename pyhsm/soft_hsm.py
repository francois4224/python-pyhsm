...
class _ctr_counter:
    ...
    def pack(self):
        ...
        return struct.pack(
            fmt, self.flags, self.key_handle, self.nonce, 0, 0, 0, val  # rfu
        )
...
class _cbc_mac:
    def __init__(self, key, key_handle, nonce, data_len):
        ...
        """
        flags = (((pyhsm.defines.YSM_AEAD_MAC_SIZE - 2) / 2) << 3) | (
            pyhsm.defines.YSM_CCM_CTR_SIZE - 1
        )
...
def aesCCM(key, key_handle, nonce, data, decrypt=False):
    ...
    """
    if decrypt:
        (data, saved_mac) = _split_data(
            data, len(data) - pyhsm.defines.YSM_AEAD_MAC_SIZE
        )
...
class SoftYHSM(object):
    def __init__(self, keys, debug=False):
        ...
        for k, v in keys.items():
            if len(v) not in AES.key_size:
                raise ValueError(
                    "Keyhandle of unsupported length: %d (was %d bytes)" % (k, len(v))
                )
        self.keys = keys
    ...
    def _get_key(self, kh, cmd):
        try:
            ...
            raise pyhsm.exception.YHSM_CommandFailed(
                pyhsm.defines.cmd2str(cmd), pyhsm.defines.YSM_KEY_HANDLE_INVALID
            )
    ...
    def generate_aead(self, nonce, key_handle):
        ...
        ct = pyhsm.soft_hsm.aesCCM(aes_key, key_handle, nonce, self._buffer, False)
