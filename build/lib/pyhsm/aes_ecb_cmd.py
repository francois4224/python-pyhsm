...
class YHSM_Cmd_AES_ECB(YHSM_Cmd):
    ...
    def parse_result(self, data):
        ...
        key_handle, result, self.status = struct.unpack(fmt, data)
...
class YHSM_Cmd_AES_ECB_Encrypt(YHSM_Cmd_AES_ECB):
    ...
    def __init__(self, stick, key_handle, plaintext):
        pyhsm.util.input_validate_str(
            plaintext, name="plaintext", max_len=pyhsm.defines.YSM_BLOCK_SIZE
        )
        ...
        payload = struct.pack("<I", key_handle) + plaintext.ljust(
            pyhsm.defines.YSM_BLOCK_SIZE, chr(0x0).encode()
        )
        YHSM_Cmd_AES_ECB.__init__(
            self, stick, pyhsm.defines.YSM_AES_ECB_BLOCK_ENCRYPT, payload
        )
...
class YHSM_Cmd_AES_ECB_Decrypt(YHSM_Cmd_AES_ECB):
    ...
    def __init__(self, stick, key_handle, ciphertext):
        self.key_handle = pyhsm.util.input_validate_key_handle(key_handle)
        pyhsm.util.input_validate_str(
            ciphertext, name="ciphertext", exact_len=pyhsm.defines.YSM_BLOCK_SIZE
        )
        ...
        YHSM_Cmd_AES_ECB.__init__(
            self, stick, pyhsm.defines.YSM_AES_ECB_BLOCK_DECRYPT, payload
        )
...
class YHSM_Cmd_AES_ECB_Compare(YHSM_Cmd_AES_ECB):
    ...
    def __init__(self, stick, key_handle, ciphertext, plaintext):
        ...
        YHSM_Cmd_AES_ECB.__init__(
            self, stick, pyhsm.defines.YSM_AES_ECB_BLOCK_DECRYPT_CMP, payload
        )
    ...
    def parse_result(self, data):
        ...
        fmt = "< I B"
        key_handle, self.status = struct.unpack(fmt, data)
