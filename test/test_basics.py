...
class TestBasics(test_common.YHSM_TestCase):
    ...
    def test_load_temp_key(self):
        ...
        my_flags = struct.pack(
            "< I", 0xFFFFFFFF
        )  # full permissions when loaded into phantom key handle
        ...
        plaintext = "Testing".ljust(
            pyhsm.defines.YSM_BLOCK_SIZE
        )  # pad for compare after decrypt
        ciphertext = self.hsm.aes_ecb_encrypt(
            pyhsm.defines.YSM_TEMP_KEY_HANDLE, plaintext
        )
        self.assertNotEqual(plaintext, ciphertext)

        # Now decrypt it again and verify result
        decrypted = self.hsm.aes_ecb_decrypt(
            pyhsm.defines.YSM_TEMP_KEY_HANDLE, ciphertext
        )
