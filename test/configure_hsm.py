...
class ConfigureYubiHSMforTest(test_common.YHSM_TestCase):
    ...
    def test_configure_YHSM(self):
        ...
        if not self.hsm.version.have_key_store_decrypt():
            ...
            AdminYubiKeysStr += "\r"
            self.config_do(
                "hsm ffffffff\r%s\r%s\r%s\ryes"
                % (CfgPassphrase, AdminYubiKeysStr, HsmPassphrase)
            )
        ...
        escape_char = chr(27)
        self.config_do(
            "dbload\r00001,%s,%s,%s,\r" % (PrimaryAdminYubiKey) + escape_char,
            add_cr=False,
        )
    ...
    def test_zzz_unlock(self):
        ...
        if self.hsm.version.have_unlock():
            ...
            YK.use_ctr = 2
            self.assertTrue(
                self.hsm.unlock(password=HsmPassphrase.decode("hex"), otp=YK.from_key())
            )
        else:
