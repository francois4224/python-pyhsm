...
class TestOath(test_common.YHSM_TestCase):
    ...
    def test_OATH_HOTP_values(self):
        ...
        test_vectors = [
            ...
            (
                30,
                "543c61f8f9aeb35f6dbc3a6847c3fe288cc0ee4c",
                26920,
            ),
        ]

        for c, expected, code in test_vectors:
            hmac_result = self.hsm.hmac_sha1(
                self.phantom, struct.pack("> Q", c)
            ).get_hash()
    ...
    def test_OATH_HOTP_validation(self):
        ...
        oath = (
            lambda counter, user_code, look_ahead: pyhsm.oath_hotp.search_for_oath_code(
                self.hsm,
                self.key_handle,
                self.nonce,
                self.aead,
