...
class TestAEAD(test_common.YHSM_TestCase):
    ...
    def test_generate_aead_simple_validates(self):
        ...
        self.assertTrue(
            self.hsm.validate_aead(
                aead.nonce, kh_val, aead, cleartext=self.secret.pack()
            )
        )
    ...
    def test_generate_aead_random_without_nonce(self):
        ...
        for num_bytes in (
            ...
            self.assertEqual(
                num_bytes + pyhsm.defines.YSM_AEAD_MAC_SIZE, len(aead.data)
            )
    ...
    def test_who_can_validate(self):
        ...
        this = lambda kh: self.hsm.validate_aead(
            aead.nonce, kh, aead, cleartext=self.secret.pack()
        )
