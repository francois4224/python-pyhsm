...
class TestHMACSHA1(test_common.YHSM_TestCase):
    ...
    def test_nist_test_vector(self):
        ...
        self.assertEquals(
            this.get_hash().encode("hex"), "0922d3405faa3d194f82a45830737d5cc6c75d24"
        )
    ...
    def test_sha1_to_buffer(self):
        ...
        self.assertEqual(
            0, self.hsm.load_random(0, offset=0)
        )  # offset = 0 clears buffer

        self.hsm.hmac_sha1(self.kh, "testing is fun!", to_buffer=True)
        # Verify there is now 20 bytes in the buffer
        self.assertEqual(
            pyhsm.defines.YSM_SHA1_HASH_SIZE, self.hsm.load_random(0, offset=1)
        )
