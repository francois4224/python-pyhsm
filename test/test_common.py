...
class YHSM_TestCase(unittest.TestCase):
    ...
    # def setUp(self, device=os.getenv("YHSM_DEVICE", "/dev/ttyACM0"), debug=False): TODO
    def setUp(
        self,
        device=os.getenv("YHSM_DEVICE", "/dev/tty.usbmodem8D89566B52511"),  #
        debug=False,
    ):
        """
    ...
    def who_can(self, what, expected=[], extra_khs=[]):
        ...
        for kh in list(range(1, 32)) + extra_khs:
            ...
            try:
                res = what(kh)
                self.fail(
                    "Expected YSM_FUNCTION_DISABLED for key handle 0x%0x, got '%s'"
                    % (kh, res)
                )
            except pyhsm.exception.YHSM_CommandFailed as e:
    ...
    def otp_unlock(self):
        ...
        YK = FakeYubiKey(
            pyhsm.yubikey.modhex_decode(Params[0]).decode("hex"),
            Params[1].decode("hex"),
            Params[2].decode("hex"),
        )
        YK.session_ctr = 0
...
class YubiKeyEmu:
    ...
    def pack(self):
        ...
        res = struct.pack(
            fmt, self.user_id, self.use_ctr, ts_low, ts_high, self.session_ctr, self.rnd
        )
