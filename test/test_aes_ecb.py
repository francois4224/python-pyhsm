...
class TestOtpValidate(test_common.YHSM_TestCase):
    ...
    def test_encrypt_decrypt(self):
        ...
        plaintext = b"Fjaellen 2011".ljust(
            pyhsm.defines.YSM_BLOCK_SIZE
        )  # pad for compare after decrypt
    ...
    def test_compare(self):
        ...
        self.assertTrue(
            self.hsm.aes_ecb_compare(self.kh_compare, ciphertext, plaintext)
        )
        self.assertFalse(
            self.hsm.aes_ecb_compare(self.kh_compare, ciphertext, plaintext[:-1] + "x")
        )
    ...
    def test_aes_with_keystore_locked(self):
        ...
        try:
            ...
            self.fail(
                "Expected YSM_KEY_STORAGE_LOCKED, got %s (before lock: %s)"
        ...
        self.assertTrue(
            self.hsm.key_storage_unlock(test_common.HsmPassphrase.decode("hex"))
        )
