...
class TestYubikeyValidate(test_common.YHSM_TestCase):
    ...
    def test_validate_aead_cmp(self):
        ...
        self.assertTrue(
            self.hsm.validate_aead(
                self.yk_public_id, self.kh_validate, self.aead, cleartext
            )
        )
        wrong_cleartext = "X" + cleartext[1:]
        self.assertFalse(
            self.hsm.validate_aead(
                self.yk_public_id, self.kh_validate, self.aead, wrong_cleartext
            )
        )
    ...
    def test_validate_aead_cmp_long(self):
        ...
        wrong_cleartext = "X" + cleartext[1:]
        self.assertFalse(
            self.hsm.validate_aead(nonce, key_handle, aead, wrong_cleartext)
        )
    ...
    def test_modhex_encode_decode(self):
        ...
        self.assertEquals(
            h, pyhsm.yubikey.modhex_decode(pyhsm.yubikey.modhex_encode(h))
        )
    ...
    def test_split_id_otp(self):
        ...
        (
            public_id,
            otp,
        ) = pyhsm.yubikey.split_id_otp("ft" * 16)
        ...
        (
            public_id,
            otp,
        ) = pyhsm.yubikey.split_id_otp("cc" + "ft" * 16)
